...
class DiceCoefficient(object):
    ...
    def update(self, pred, target):
        ...
        # pred.argmax--针对每一个像素找到它所属概率最大的一个类别的预测数值
        # 将它转化成one_hot编码的形式
        # 进行一个permute处理，将channel维度移到索引为1的位置得到pred
        pred = F.one_hot(pred.argmax(dim=1), self.num_classes).permute(0, 3, 1, 2).float()
        # 同样的方法调用build_target函数将target也转化成one_hot编码的形式
        dice_target = build_target(target, self.num_classes, self.ignore_index)
        # 调用multiclass_dice_coeff函数计算dice coeff
        # 注意这里计算忽略掉背景了，从channel 1开始计算，0是背景维
    ...
    def value(self):
        if self.count == 0:
            ...
            # 累积的dice coeff除以累积的样本个数self.count，得到dice coeff的均值
